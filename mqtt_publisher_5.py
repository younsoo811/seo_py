    ...
    for j in range(0,int(entry_time)):
        ...
        for i in range(0,int(entry_size)):
            if check1.get()==1:
                ...
                # objects
                len_dic=len(framedata_json["objects"])
                if len_dic>1:
                    f_list=[]
                    for k in range(len_dic):
                        f_list.append(framedata_json["objects"][k])
                    #print("테스트: ",f_list[2])
                    for index in range(len_dic):
                        sub_list=[]
                        sub_list.append(f_list[index])
                        framedata_json["objects"]=sub_list #??
                        print(framedata_json["objects"])
